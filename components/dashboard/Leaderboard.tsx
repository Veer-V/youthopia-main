...
const Leaderboard: React.FC<LeaderboardProps> = ({ bonus = 0 }) => {
  const { users, user, isInitializing } = useData();
  ...
  const leaders = users
    ...
    .map(u => ({
      ...
      points: u.points || 0,
      school: u.institute,
      category: getCategory(u.points || 0)
    }))
    // Deduplicate by name, preferring entries with school/institute info
    .reduce((acc: any[], current) => {
      const x = acc.find(item => item.name === current.name);
      if (!x) {
        return acc.concat([current]);
      } else {
        // If current has school and existing doesn't, update existing
        if (!x.school && current.school) {
          x.school = current.school;
          x.points = Math.max(x.points, current.points); // Also take max points just in case
          x.category = getCategory(x.points);
        }
        return acc;
      }
    }, []);
  ...
  // No transient loader for leaderboard — render directly on filter/view changes
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {categories.map((cat) => (
          <button
            ...
            className={`relative px-4 py-2 rounded-full text-sm font-bold transition-all border flex items-center gap-2 overflow-hidden ${activeCategory === cat
            ...
            >
              {filteredLeaders.length > 0 ? (
                filteredLeaders.map((l) => {
                  const isCurrentUser = users.find(u => u.name === l.name && (u.email === user?.email || u.id === user?.id)) !== undefined;
                  ...
                  return (
                    <motion.div
                      ...
                      whileHover={{
                        scale: 1.005,
                        backgroundColor: isCurrentUser ? "rgba(254, 252, 232, 1)" : "rgba(248, 250, 252, 1)", // Hover: Yellow-50 if current, Slate-50 otherwise
                      }}
                      className={`grid grid-cols-12 p-4 items-center cursor-default transition-colors ${isCurrentUser ? 'bg-yellow-50 border-l-4 border-yellow-400' : ''
                        }`}
                    >
                      <div className="col-span-2 text-center font-bold text-lg text-slate-400">
                        {isCurrentUser && <span className="mr-2 text-yellow-500">👉</span>}
                      ...
                      <div className="col-span-6 md:col-span-5">
                        <div className="font-bold text-slate-800 flex items-center gap-2">
                          {l.name}
                          {isCurrentUser && <span className="px-2 py-0.5 bg-yellow-200 text-yellow-800 text-[10px] rounded-full font-bold uppercase">You</span>}
                        </div>
