...
import { Trophy, Search, Medal, Crown, Star, Zap, List, BarChart3, Loader } from 'lucide-react';
...
const Leaderboard: React.FC<LeaderboardProps> = ({ bonus = 0 }) => {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        setLoading(true);
        const result = await API.getAllLeaderboard();
        if (result.data) {
  ...
  // Convert API data to Leaders List
  const leaders = leaderboardData.map(entry => ({
    name: entry.name,
    points: entry.points || 0,
    school: entry.Yid || 'N/A',
    category: getCategory(entry.points || 0)
  }));
  ...
  return (
    <div>
      ...
      {/* Loading State */}
      {loading && (
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-12 flex flex-col items-center justify-center min-h-[400px]">
          <Loader className="animate-spin text-brand-purple mb-4" size={48} />
      ...
      {/* Error State */}
      {error && !loading && (
        <div className="bg-red-50 rounded-3xl border border-red-200 p-8 flex flex-col items-center justify-center min-h-[200px]">
          <Trophy className="text-red-400 mb-4" size={48} />
