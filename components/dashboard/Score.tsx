...
const Score: React.FC<ScoreProps> = ({ bonus, registeredEventIds = [] }) => {
  const { user } = useData();
  const eventCount = user?.registered ? Object.keys(user.registered).length : 0;
  const pointsReceived = user?.points || 0;
  // Performance data with event participation and points received
  const scores: { title: string; score: number; color: string }[] = [
    { title: "Events Registered", score: eventCount, color: "bg-brand-purple" },
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="text-center md:text-left">
            ...
            <h3 className="text-6xl font-black mb-1">
              <AnimatedCounter value={pointsReceived} />
            </h3>
          ...
          {scores.length > 0 ? scores.map((item, idx) => (
            <div key={idx}>
              ...
              <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden relative">
                <motion.div
                  initial={{ width: "0%" }}
                  animate={{ width: item.title === "Events Registered" ? `${Math.min(100, (item.score / 20) * 100)}%` : `${Math.min(100, (item.score / 1000) * 100)}%` }}
