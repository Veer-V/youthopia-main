...
const AuthPage: React.FC<AuthPageProps> = ({ onBack, onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (activeTab === 'register') {
        ...
        const result = await addUser(newUser, formData.password);

        if (result.success && result.user) {
          // Save the API response data to localStorage via onLogin
          onLogin(result.user, result.user.points || 5);
      ...
      if (activeTab === 'login') {
        ...
        const { user, error: loginError } = await login(formData.mobile, formData.password);
  ...
  const getHeaderSubtitle = () => {
    switch (activeTab) {
      case 'login': return 'Sign in with your mobile number.';
