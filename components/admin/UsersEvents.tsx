...
const UsersEvents: React.FC = () => {
   ...
   const handleAddEvent = () => {
      if (editingEventId) {
         // Update Logic
         const evtToUpdate = events.find(e => e.id === editingEventId);
         if (evtToUpdate) {
            updateEvent({
               ...evtToUpdate,
      ...
      <div className="space-y-6">
         ...
         {showEventModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
               <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl">
                  <div className="flex justify-between items-center mb-6">
                     <h3 className="text-xl font-bold text-slate-800">{editingEventId ? 'Update Event' : 'Create Event'}</h3>
                  ...
                  <Button fullWidth onClick={handleAddEvent} className="mt-4">{editingEventId ? 'Update Event' : 'Publish Event'}</Button>
               </div>
            </div>
